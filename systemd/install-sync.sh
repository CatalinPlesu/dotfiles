...
while read -r REPO; do
    ...
    if [[ -d "$REPO/.git" ]]; then
        ...
        # Bail if repo is mid-rebase/merge from previous run
        if [[ -d .git/rebase-merge || -d .git/rebase-apply || -f .git/MERGE_HEAD ]]; then
            echo "[$LOG_TAG] $REPO has unfinished rebase/merge — skipping" >&2
            continue
        fi
        ...
        if ! git diff-index --quiet HEAD -- 2>/dev/null; then
            ...
            # Nothing changed since last commit — don't commit again
            git pull --rebase && git push 2>/dev/null
            continue
        fi

        # Pull with merge (not rebase) — on conflict, markers stay in files
        if ! git pull --no-rebase; then
            echo "[$LOG_TAG] Merge conflict in $REPO — committing conflict markers" >&2
            git add .
            git commit -m "Auto-sync CONFLICT [$(date '+%Y-%m-%d %H:%M:%S')] on $(hostname)"
        fi

        git push || echo "[$LOG_TAG] Push failed for $REPO" >&2
    fi
done < "$HOME/.config/systemd/user/repo-list"
OUTER_EOF
    chmod +x "$SYNC_SCRIPT"
